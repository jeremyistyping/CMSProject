...
func main() {
	...
	fmt.Println("🔧 FIXING SALES JOURNAL INTEGRATION")
	...
	for rows.Next() {
		...
		err = db.QueryRow(`
			...
			WHERE reference_type = 'SALES' AND reference_id = $1
	...
	err = db.QueryRow(`
		...
		WHERE a.code = '4101' AND ujd.reference_type = 'SALES'
	`).Scan(&totalRevenue)
...
func createSalesJournalEntry(db *sql.DB, saleID int, code, invoiceNumber string, totalAmount, subtotal, ppn float64, saleDate time.Time) error {
	...
	err = tx.QueryRow(`
		INSERT INTO unified_journal_ledger (
			code, date, description, reference_type, reference_id, 
			total_amount, status, created_at, updated_at
		) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id
	`, 
		fmt.Sprintf("JRN-SALES-%s", code), 
		saleDate,
	...
	_, err = tx.Exec(`
		INSERT INTO unified_journal_lines (
			journal_id, account_id, debit_amount, credit_amount, description, created_at, updated_at
		) VALUES ($1, $2, $3, $4, $5, $6, $7)
	`, journalID, arAccountID, totalAmount, 0, fmt.Sprintf("AR - Invoice %s", invoiceNumber), time.Now(), time.Now())
	if err != nil {
	...
	`, journalID, revenueAccountID, 0, revenueAmount, fmt.Sprintf("Sales Revenue - Invoice %s", invoiceNumber), time.Now(), time.Now())
	if err != nil {
	...
	if ppn > 0 {
		...
		`, journalID, ppnAccountID, 0, ppn, fmt.Sprintf("PPN 11%% - Invoice %s", invoiceNumber), time.Now(), time.Now())
		if err != nil {
